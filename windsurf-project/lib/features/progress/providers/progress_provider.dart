...
class ProgressProvider with ChangeNotifier {
  ...
  Future<void> loadProgressData(String userId) async {
    ...
    try {
      await Future.wait([
        _loadProgressEntries(userId),
        _loadWeightHistory(userId),
        _loadProgressPhotos(userId),
      ]);
    } catch (e) {
  ...
  Future<void> _loadProgressEntries(String userId) async {
    _progressEntries = await _firestoreService.getUserProgress(userId).first;
    notifyListeners();
  }
  ...
  Future<void> addProgressEntry(ProgressEntry entry) async {
    try {
      ...
      await _loadProgressEntries(entry.userId);
    } catch (e) {

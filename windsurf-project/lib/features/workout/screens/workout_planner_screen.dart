...
class _WorkoutPlannerScreenState extends State<WorkoutPlannerScreen> {
  ...
  Future<void> _savePlan() async {
    ...
    // Support guest/open-and-run mode by falling back to userModel.id
    final userId = authProvider.user?.uid ?? authProvider.userModel?.id;
    if (userId == null) return;

    // Reuse existing plan id if present so we update instead of always creating a new one
    final existingPlan = workoutProvider.workoutPlan;
    final planId = existingPlan?.id ?? DateTime.now().millisecondsSinceEpoch.toString();

    final plan = WorkoutPlan(
      id: planId,
      userId: userId,
      weeklyPlan: Map<int, String>.from(_weeklyPlan),
    );
